...
#importa la lista de tokens y comprueba que tipo de token es el que se requiere analizar
#una vez encontrado el tipo de token se mandan al yacc y este comprueba que este correcto el token de manera sintactica si enccuentra un error despliega p_error(p)
def p_program(p):
...
#codigo que se despliega en caso que el yacc encuentre un error en el o los tokens 
def p_error(p):
    #print str(dir(p))
